...
### A. `confirm.html` 작동 확인 — 아직 한 번도 안 됨

만들기만 했다. Pages 주소에서 새 계정으로 가입 → 메일 링크 클릭 →
"이메일 인증이 완료되었습니다" + 「로그인하기」 버튼이 나오는지 확인해야 한다.
Pages 주소에서 가입하면 메일 링크도 Pages 주소로 가서 휴대폰에서도 열린다.

### B. Pages 주소에서 전체 작동 확인

로그인 → 카드 뽑기 → 새로고침해도 이력 남는지. 모바일에서도.

### C. 시연 녹화 (3분)

필수: 로그인 전 접근 불가 → 로그인 → 뽑기 → **F5 후에도 이력 남는 것** →
로그아웃·재로그인 → 다른 계정에서 안 보이는 것

넣으면 강한 것: F12 Network 에 `interpret` 만 있고 `api.anthropic.com` 이 없는 것
(⚠️ Network 탭은 **열어둔 뒤** 일어난 요청만 기록한다. 뽑기 전에 열어야 한다),
Supabase 대시보드의 테이블·정책·Secrets

⚠️ 개발자도구를 열어둔 채로 전체를 녹화하면 앱이 안 보인다. 마지막 20초만.
시크릿 창(Ctrl+Shift+N)으로 녹화하면 캐시·저장된 비밀번호가 없어 깨끗하다.

### D. 해석 톤 — 판단 보류 중

"포장하지 말 것 / 안심시키지 말 것" 두 마디만 있고 좋게 나올 때 좋다고 쓰라는 말이 없어서
한쪽으로 기울 수 있다. 다만 뽑힌 카드가 실제로 그런 카드였을 수도 있다.
**몇 번 더 뽑아보고 판단하기로 했다. 지시 없이 프롬프트를 덧대지 말 것.**

### E. 마무리

| 항목 | 상태 |
|---|---|
| 무료 티어 한도·요금 알림 | ⬜ Anthropic Console 에서 설정 |
| `sbp_` 토큰 삭제 | ⬜ 배포 끝났으니 지울 수 있다 |

### F. 회고 4문항 (과제에 포함)

1. 내 화면이 세상과 끊겨 있던 이유를 손님·주방 비유로 한 문장
2. 404와 500 중 더 만나고 싶지 않은 것과 이유
3. '먼저 예측' 표에서 틀리게 예측했던 항목
4. AI에게 보안을 맡길 때 반드시 되물어야 할 질문 하나

→ 사용자가 직접 써야 하는 것. **대신 쓰지 말 것.**

---

## 다음 창에서 바로 할 일 (순서)

1. `confirm.html` 작동 확인 (위 A)
2. Pages 주소에서 전체 작동 확인, 모바일 포함
3. 시연 녹화
4. 요금 알림 설정, `sbp_` 토큰 삭제
